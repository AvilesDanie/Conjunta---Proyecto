...
@WebServlet("/banquito/creditos/cuotas/pagar")
public class BanquitoPagarCuotaController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idCredito == null || idCredito.isBlank()
            ...
            registrarError(session, "No se pudo identificar la cuota seleccionada.");
        ...
        try {
            ...
            if (status >= 200 && status < 300) {
                registrarExito(session, "Cuota " + (numeroCuota != null ? "#" + numeroCuota + " " : "")
                        + "pagada correctamente.");
            } else {
                String detalle = leerMensaje(conn);
                registrarError(session,
                        "No se pudo pagar la cuota (HTTP " + status + "). "
                                + (detalle.isBlank() ? "" : detalle));
            }
        } catch (IOException ex) {
            ex.printStackTrace();
            registrarError(session, "Ocurrió un error de comunicación al pagar la cuota.");
        } finally {
            if (conn != null) {
                conn.disconnect();
            }
    ...
    private String leerMensaje(HttpURLConnection conn) throws IOException {
